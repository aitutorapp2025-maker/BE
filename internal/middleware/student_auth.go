...
// SignedStudent authenticates a student (mobile) request with the same strong
// scheme as SignedAdmin: a valid student JWT (with session id), a fresh
// timestamp, an HMAC signature over METHOD\nURL\nsha256(body)\nnonce\ntimestamp
// using the session signing secret, and a one-time nonce (replay-protected).
// Every request therefore carries a unique, single-use credential.
func SignedStudent(cfg config.Config, store *session.Store) fiber.Handler {
	return func(c *fiber.Ctx) error {
		header := c.Get("Authorization")
		if !strings.HasPrefix(header, "Bearer ") {
			return fiber.NewError(fiber.StatusUnauthorized, "missing bearer token")
		}
		claims, err := auth.ParseStudentToken(cfg.JWT.Secret,
