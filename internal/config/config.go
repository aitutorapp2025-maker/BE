...
// UseGemini reports whether tutor answers are generated by Gemini.
func (a AIConfig) UseGemini() bool { return a.AnswersProvider == "gemini" }

// AnswersKey is the API key of the active answers provider.
func (a AIConfig) AnswersKey() string {
	if a.UseGemini() {
		return a.GeminiKey
	}
	return a.AnthropicKey
}

// Enabled reports whether the tutoring pipeline has the keys it needs: an
// answers key for the active provider plus a working embeddings backend
// (Voyage key, or a local embeddings endpoint).
func (a AIConfig) Enabled() bool {
	embedOK := a.VoyageKey != "" || (a.EmbedProvider == "local" && a.LocalEmbedURL != "")
	return a.AnswersKey() != "" && embedOK
}
