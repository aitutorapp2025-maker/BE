...
func main() {
	...
	registrations := []struct {
		...
		{scheduler.AutopayNudgeJob(studentRepo, deviceRepo, pushSender),
			"AutoPay reminder push",
			"Once a day, pushes a friendly nudge to every student who has NOT enabled UPI AutoPay: start the AI tutor for just Rs.5 - daily practice brings higher scores. Time it from this panel."},
		{scheduler.TaskTimeUpJob(homeworkRepo, deviceRepo, pushSender),
			"Homework time's-up alerts",
			"Every minute, pushes 'Time's up' when a started task's planned minutes run out and it still isn't completed — calls the student back to finish."},
